...
def get_spk_real(df_flt, df_speak_all, thre):
    ...
    idxs_mean = [False if u not in potential_spks or np.isnan(df_comb.ix[i]['%s_vol' % u]) else (df_comb.ix[i][df_comb.ix[i][volume_spks][df_comb.ix[i][volume_spks] > df_comb.ix[i]['%s_vol' % u]].index.map(
        lambda x: x.rstrip('_vol'))] < thre).all() for i, u in enumerate(df_comb.index.get_level_values('member'))]
